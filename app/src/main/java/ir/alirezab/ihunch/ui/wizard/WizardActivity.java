package ir.alirezab.ihunch.ui.wizard;
...
public class WizardActivity extends AppCompatActivity {
	...
	private final int[] about_images_array = {
			R.drawable.body_weight_on_neck,
			R.drawable.avatar,
			R.drawable.body_weight_on_neck,
			R.drawable.avatar
	};
...
//        Tools.setSystemBarLight(this);
	...
	private void bottomProgressDots (int current_index) {
		...
		for (int i = 0; i < dots.length; i++) {
			...
			dots[i].setColorFilter (getResources ().getColor (R.color.grey_20, getTheme ()), PorterDuff.Mode.SRC_IN);
			dotsLayout.addView (dots[i]);
		}
		
		if (dots.length > 0) {
			dots[current_index].setImageResource (R.drawable.shape_circle);
			dots[current_index].setColorFilter (getResources ().getColor (R.color.light_green_600, getTheme ()), PorterDuff.Mode.SRC_IN);
		}
	}
